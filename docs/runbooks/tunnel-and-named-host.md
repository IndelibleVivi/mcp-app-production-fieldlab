...
This document defines external evidence contracts. It is **not** an executable default and does not authorize credentials access, tunnel/service changes, deployment, authenticated host interaction or owner acceptance. The corresponding scenarios have `runner: null`.

Current status for tunnel, named host and owner is `not_verified`.

## Gates and required authority

| Gate                        | Minimum authority                                                                      | Side effects                                                                  |
| --------------------------- | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| Configured-target preflight | Operator explicitly authorizes use of the selected credential/profile and target       | Reads private config; may contact the exact loopback runtime                  |
...
- exact runtime candidate/bundle digest and, if applicable, immutable image/service identity;
...
If these values are ambiguous, stop. Do not infer a port, host alias, credential file or active service from an example.
...
- target is loopback HTTP with the exact MCP path;
...
- no alternate port or config source silently wins.
...
Passing this preflight can verify configured-target authority for that exact config/runtime pair. It does **not** prove the tunnel process is running, public reachability, named-host discovery/resource admission or owner acceptance.
...
5. re-run configured-target preflight against the same profile;
...
- observed capability discovery and exact disposition (`success`, `rejected`, `cancelled`, `policy_denied`, `technical_failure`, and so on);
...
- capability result remains separate from the overall claim status;
